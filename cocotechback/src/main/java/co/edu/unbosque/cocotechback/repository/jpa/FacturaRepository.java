 ...
 * métodos de consulta personalizados para buscar facturas por venta, rango
 * de fechas y rango de valores.
 * <p>
...
public interface FacturaRepository extends JpaRepository<Factura, Long> {
	 ...
	 * <p>
	 * La relación venta-factura es 1:1, por lo tanto este método retorna a lo
	 * sumo una factura por venta.
	 *
	 * @param idVenta El identificador de la venta cuya factura se desea consultar.
	 * @return Un {@link Optional} con la factura de la venta, o vacío si la
	 *         venta aún no tiene factura generada.
	 */
	 ...
	 * @return Una lista de facturas emitidas dentro del rango indicado. Retorna
	 ...
	 * @return Una lista de facturas dentro del rango de precio total. Retorna
	 *         una lista vacía si no hay coincidencias.
	 */
	public List<Factura> findByPrecioTotalBetween(Double minTotal, Double maxTotal);

	/**
	 * Busca todas las facturas de un cliente específico, navegando a través de
	 * la relación Factura → Venta → Cliente.
	 *
	...
	/**
	 * Consulta JPQL que calcula el total de impuestos recaudados dentro de un
	 * rango de fechas.
	 * <p>
	 ...
	 * @return El total de impuestos recaudados en el periodo, o {@code null}
	 *         si no hay facturas en el rango.
	 */
	...
	/**
	 * Consulta JPQL que calcula el ingreso bruto total del supermercado dentro
	 * de un rango de fechas (suma de todos los precios totales de facturas).
	 *
	 * @param inicio La fecha y hora de inicio del rango.
	 * @param fin    La fecha y hora de fin del rango.
	 * @return El ingreso bruto total del periodo, o {@code null} si no hay
	 *         facturas en el rango.
	 */
	@Query("SELECT SUM(f.precioTotal) FROM Factura f WHERE f.fecha BETWEEN :inicio AND :fin")
	public Double findIngresoBrutoPorPeriodo(LocalDateTime inicio, LocalDateTime fin);
}
